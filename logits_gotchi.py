...
PROMPT_METHODOLOGY = "No Instructions"
...
def llm_chat_completion(messages:list, model=MODEL, **kwargs):
    ...
    kwargs.update({
        'model': model,
        'messages': messages})
    return requests.post("http://192.168.1.100:8853/v1d/chat/completions", json=kwargs).json()

def llm_tokenize(text):
...
"""
    ...
    def llm_round(self):
        ...
        if self.messages is None:
            self.messages = msgs(self.pet_msg(), sysprompt=self.prompt)
        else:
            self.messages.append({"role": "user", "content": self.pet_msg()}) # context!
        ...
        # extract top logprobs from response
        tlp = llm_output['choices'][0]['logprobs']['top_logprobs'][0]
        
        # softmax to probabilities
        probs = token_probs(tlp)
        
        # action is the highest value
        action = sorted(probs.keys(), key = lambda i: probs[i], reverse = True)[0]
        #print(probs, '->', action)
